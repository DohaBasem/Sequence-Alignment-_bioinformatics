#This is the dynamic programming script for  Local sequence allignment
...
my @MAX=(0,0,0);		#This array holds the maximum score of last row/col @Max=(max_score,row#,col#)
	...
	#In the local allignmnet since we are ignoring the \ gaps in both sequences
	#So the termination point is the maximum 
	...
	#Since this is a local allignment problem ,so termination is at the cell  with max value
	while( $ScoresMatrix[$R][$C]!=0)
	{	
		
		
		if($ScoresMatrix[$R][$C]-$gap==$ScoresMatrix[$R-1][$C])
		{	
			push(@alligned1,"_");
